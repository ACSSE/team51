...
using Bursify.Migrations.SeedData;
...
namespace Bursify.Tests
{
    public abstract class BursifyTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<BursifyDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<BursifyDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BursifyDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BursifyDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BursifyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BursifyDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<BursifyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BursifyDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BursifyDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BursifyDbContext, Task<T>> func)
        {
            T result;
